...
# Mock the app imports to avoid database connection during tests
...
class TestAppStructure:
    ...
    def test_templates_exist(self):
        ...
        for template in templates:
            assert os.path.exists(f'templates/{template}'), f"Template {template} not found"
    
    def test_static_css_exists(self):
        """Test that static CSS files exist"""
        assert os.path.exists('static/css/style.css'), "CSS file not found"
...
class TestRequirements:
    ...
    def test_pymysql_version(self):
        ...
        with open('requirements.txt', 'r') as f:
            content = f.read()
            assert 'PyMySQL' in content, "PyMySQL not in requirements.txt"
    
    def test_werkzeug_version(self):
        """Test Werkzeug is installed"""
        with open('requirements.txt', 'r') as f:
            content = f.read()
            assert 'Werkzeug' in content, "Werkzeug not in requirements.txt"
